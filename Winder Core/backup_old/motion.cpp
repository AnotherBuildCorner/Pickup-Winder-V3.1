...
AxisStepper::AxisStepper(gpio_num_t stepPin, gpio_num_t dirPin, gpio_num_t enablePin)
...
void AxisStepper::begin(bool clockwise) {
  ...
  if (_enablePin != GPIO_NUM_NC) {
    ...
    gpio_set_level(_enablePin, !_run);
  }
...
void AxisStepper::setRate(float stepsPerSecond) {
  _targetRate = stepsPerSecond;
}

void AxisStepper::setAcceleration(float stepsPerSec2) {
  _accelPerTick = (stepsPerSec2 * SCALE) / (ISR_FREQ * ISR_FREQ);
}

void AxisStepper::tick() {
  if (!_run) return;
  ...
  if (_accum >= SCALE) {
    ...
    if (!_stepState) { // Only count one edge
  _stepCount++;
}
  }
}

int AxisStepper::getStepCount() const {
  return _stepCount;
}

float AxisStepper::getTurnCount() const {
  return static_cast<float>(_stepCount) / (2.0f * steps_rev);  // steps per full cycle
}
...
TraverseStepper::TraverseStepper(gpio_num_t stepPin, gpio_num_t dirPin, gpio_num_t enablePin)
  : _stepPin(stepPin), _dirPin(dirPin), _accum(0), _rate(0),
    _stepState(false), _dirState(true), _run(false),
    _pos(0), _posMin(0), _posMax(1000),
    _homePin(GPIO_NUM_NC), _homeActiveLow(true), _homed(false) {}
...
void initMotionPins() {
  Serial.println("initilaizing Motion Pins...");
  pinMode((gpio_num_t)SPINDLE_STEP_PIN, OUTPUT);
  gpio_set_level((gpio_num_t)SPINDLE_STEP_PIN, LOW);
  Serial.println("Initializing core timers");

  timer = timerBegin(0, 80, true);          // 80MHz / 80 = 1 µs ticks
  timerAttachInterrupt(timer, &onTimer, true);
  timerAlarmWrite(timer, ISR_delay, true);          // 🔥 10µs interval
  timerAlarmEnable(timer);
  Serial.println("Motion Pins initialized successfully");
